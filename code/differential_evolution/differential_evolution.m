function [best, result] = differential_evolution(problem)
% The canonical DE algorithm
% 
% The parameter "problem" must have the following 3 members:
%     problem.lb
%     problem.ub
%     problem.solver
% 
% The following 2 members are optional
%     problem.popSize
%     problem.maxFuncCount
%  
% zyw, last modified on 20200113

problem.bound = [problem.lb' problem.ub'];
if ~isfield(problem, 'popSize')
    problem.popSize = min(length(problem.lb)*10, 100);
end
if ~isfield(problem, 'maxFuncCount')
    problem.maxFuncCount = inf;
end
...
while is_continue == 1
    globalSearch();
    if funccount >= problem.maxFuncCount
        is_continue = -1;
    end
...
if is_continue == -1
    result.msg = ['The user-specified maxFuncCount(' num2str(problem.maxFuncCount) ') is reached.'];
...
%% auxiliary functions
    ...
    function globalSearch()
        ...
        % archive
        best_id = findBestIndividual(fitness, constraint);
    ...
    function U = move(X, bound)
        ...
        indexCross = logical(rand(popSize, x_dim) <= Cr ...
            | (repmat(1:x_dim, popSize, 1)  ...
            == repmat(randi(x_dim, [popSize, 1]), 1, x_dim)));
